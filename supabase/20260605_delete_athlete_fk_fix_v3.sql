-- TAL athlete deletion FK fix v3.
-- Run this full script in Supabase SQL Editor with "Run", not "Analyze".
-- It reassigns league ownership to the acting user, or to an existing admin fallback.
...
as $tal_delete_athlete_v3$
declare
  v_current_club_id uuid;
  v_user_id uuid;
  v_reassign_user_id uuid;
begin
  ...
  into v_reassign_user_id;

  if v_reassign_user_id is null then
    raise exception 'No hay usuario admin disponible para reasignar registros.';
  end if;

  -- Reassign every known public.users foreign key before deleting this user.
  update public.indoor_leagues
  set created_by = v_reassign_user_id
  where created_by = v_user_id;

  update public.indoor_league_clubs
  set joined_by = v_reassign_user_id
  where joined_by = v_user_id;

  update public.indoor_league_coaches
  set invited_by = v_reassign_user_id
  where invited_by = v_user_id;

  update public.indoor_league_results
  set submitted_by = v_reassign_user_id
  ...
  set assigned_by = v_reassign_user_id
